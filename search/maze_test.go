...
func TestLoadMaze1(t *testing.T) {
	...
	assert.NotNil(t, maze.Start) // 0, 5
	assert.Equal(t, 0, maze.Start.X, "wrong starting x")
	assert.Equal(t, 5, maze.Start.Y, "wrong starting y")
	assert.NotNil(t, maze.Goal) // 5; 0
	assert.Equal(t, 5, maze.Goal.X, "wrong goal x")
	assert.Equal(t, 0, maze.Goal.Y, "wrong goal y")
